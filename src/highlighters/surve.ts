...
export class SurveHighlighter extends Highlighter {
    ...
    public *highlightMain(): Generator<TokenType> {
        ...
        while (this.i < this.tokens.length) {
            yield* this.highlightTopLevelStatement();
        }
    }

    // statement
    public *highlightTopLevelStatement(): Generator<TokenType> {
        if (this.peek() == "fn") {
            yield* this.highlightFunctionStatement();
            return;
        }
        if (this.peek() == "route") {
            yield* this.highlightRouteStatement();
            return;
        }
    ...
    public *highlightRouteStatement(): Generator<TokenType> {
        yield TokenType.Keyword;

        if (this.peek() == "(")
            yield TokenType.Bracket;
        
        if (["GET","HEAD","POST","PUT","DELETE","CONNECT","OPTIONS","TRACE","PATCH"].includes(this.peek())) {
